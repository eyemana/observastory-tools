module.exports = async () => {
  ...
  function formatProgressNotice(job) {
    const label = job.label ?? "Chronology Index";
    ...
    if (job.status === "succeeded") {
      return `${label} ${job.id} complete. ${job.indexed ?? 0} indexed, ${job.skipped ?? 0} skipped.`;
    }
    ...
    return `${label} ${countLabel}: ${progress.currentScene ?? "starting"}`;
  }
  ...
  const enqueueScript = path.join(toolsRoot, "scheduler", "enqueue-chronology-index.mjs");
  const workerScript = path.join(toolsRoot, "scheduler", "worker.mjs");

  try {
    new Notice("Queueing Chronology Index...");
    ...
    if (shouldLaunchWorker) {
      ...
      child.unref();
      new Notice(`Queued Chronology Index ${result.jobId}. Scheduler started.`);
    } else {
      new Notice(`Queued Chronology Index ${result.jobId}. Background scheduler will pick it up.`);
    }
  } catch (error) {
    const message = [
    ...
    new Notice("Chronology Index failed to queue. See console for details.");
  }
};
